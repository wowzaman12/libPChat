...
#if 0
static void scrollback_unlock(session *sess)
{
	char buf[1024];

	if (scrollback_get_filename(sess, buf, sizeof(buf) - 6) == nullptr)
		return;
...
static bool scrollback_lock(session *sess)
{
	char buf[1024];
	int fh;
...
static void scrollback_shrink(session *sess)
{
	...
	if (scrollback_get_filename(sess, file, sizeof(file)) == nullptr)
		return;
...
int get_stamp_str(char *fmt, time_t tim, char **ret)
{
	...
	if (len)
	{
		if (prefs.utf8_locale)
			...
			*ret = g_locale_to_utf8(dest, len, 0, (gsize*)&len, 0);
	}
...
iso_8859_1_to_utf8 (unsigned char *text, int len, gsize *bytes_written)
{
	...
	res = output = (unsigned char*)malloc((len * 3) + 1);
	if (!output)
		return nullptr;
...
char* text_validate(char **text, int *len)
{
	...
	if (prefs.utf8_locale)
		...
		utf = (char*)iso_8859_1_to_utf8((unsigned char*)*text, *len, (gsize*)&utf_len);
	else
	{
		// fallback to locale
		utf = g_locale_to_utf8(*text, *len, 0, (gsize*)&utf_len, nullptr);
		if (!utf)
			utf = (char*)iso_8859_1_to_utf8((unsigned char*)*text, *len, (gsize*)&utf_len);
	}
...
static void pevent_trigger_load(int *i_penum, char **i_text, char **i_snd)
{
	...
	if (penum != -1 && text != nullptr)
	{
		len = strlen(text) + 1;
		if (pntevts_text[penum])
			free(pntevts_text[penum]);
		pntevts_text[penum] = (char*)malloc(len);
...
void format_event(session *sess, int index, char **args, char *o, int sizeofo, unsigned int stripcolor_args)
{
	...
	char *i, *ar, d, a; bool done_all = false;
	...
	while (done_all == false)
	{
		...
		case 1:
			...
			if (ar == nullptr)
			{
				printf("arg[%d] is nullptr in print event\n", a + 1);
			}
			else
			{
...
{
	pevt_stage1 *s = nullptr, *base = nullptr, *last = nullptr, *next;
	int clen;
	...
	for (;;)
	{
		...
		if (oi > 0)
		{
			s = (pevt_stage1*)malloc(sizeof(pevt_stage1));
			if (base == nullptr)
				base = s;
			if (last != nullptr)
				last->next = s;
			last = s;
			s->next = nullptr;
			s->data = (char*)malloc(oi + sizeof(int) + 1);
...
static char* sound_find_command()
{
	// some sensible unix players. You're bound to have one of them
	static const char* const progs[] = {"aplay", "esdplay", "soxplay", "artsplay", nullptr};
	char *cmd;
	int i = 0;

	if (prefs.soundcmd[0])
		return strdup(prefs.soundcmd);
...
void sound_play(const char *file, bool quiet)
{
	char buf[512];
	char wavfile[512];
	char *file_fs;
...
#ifdef WIN32
		if (cmd == nullptr || strcmp(cmd, "esdplay") == 0)
		{
			PlaySoundA(file_fs, nullptr, SND_NODEFAULT|SND_FILENAME|SND_ASYNC);
		}
		else
#endif
		{
