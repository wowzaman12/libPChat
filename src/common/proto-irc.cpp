...
static void irc_join_list_flush(server *serv, char *c, char *k)
{
	char *chanstr, *keystr;

	chanstr = g_string_free((GString*)c, FALSE);
	keystr = g_string_free((GString*)k, FALSE);
	if (chanstr[0])
	{
		if (keystr[0])
			tcp_sendf(serv, "JOIN %s %s\r\n", chanstr, keystr);
...
static void irc_join_list(server *serv, GSList *channels, GSList *keys)
{
	GSList *clist;
	GSList *klist;
	GString *c = g_string_new(nullptr);
	GString *k = g_string_new(nullptr);
	int len;
	int add;
	int i, j;
	...
	while (clist)
	{
		...
		if (1)
		{
			add = strlen((const char*)clist->data);
			if (i != 0)
				add++;	// comma
		}
		...
		if (klist->data)
		{
			add += strlen((const char*)klist->data);
		}
		else
		{
			add++;	// 'x' filler
		...
		if (len + add > 512)
		{
			irc_join_list_flush(serv, (char*)c, (char*)k);

			c = g_string_new(nullptr);
			k = g_string_new(nullptr);
			i = j = 0;
			len = 9;
		}
		...
		if (1)
		{
			...
			g_string_append(c, (const gchar*)clist->data);
			i++;
		}
		...
		if (klist->data)
		{
			...
			g_string_append(k, (const gchar*)klist->data);
			j++;
		}
		else
		{
...
/*static void irc_get_ip(server *serv, char *nick)
{
	tcp_sendf(serv, "WHO %s\r\n", nick);
}*/
...
{
	time_t timestamp = atol(timestr);
	char *tim = ctime(&timestamp);
	tim[24] = 0;	// get rid of the \n
	EMIT_SIGNAL(XP_TE_CHANDATE, sess, chan, tim, nullptr, nullptr, 0);
}

static void process_numeric(session* sess, int n,
					char *word[], char *word_eol[], char *text)
{
	...
	case 312:
		if (!serv->skip_next_whois)
			EMIT_SIGNAL(XP_TE_WHOIS3, whois_sess, word[4], word_eol[5], nullptr, nullptr, 0);
		else
			inbound_user_info(sess, nullptr, nullptr, nullptr, word[5], word[4], nullptr, 0xff);
		break;
	...
	case 318:	// END OF WHOIS
		if (!serv->skip_next_whois)
			EMIT_SIGNAL(XP_TE_WHOIS6, whois_sess, word[4], nullptr,
							nullptr, nullptr, 0);
	...
	case 321:
		if (!fe_is_chanwindow(sess->server))
			EMIT_SIGNAL(XP_TE_CHANLISTHEAD, serv->server_session, nullptr, nullptr, nullptr, nullptr, 0);
		break;
	...
	case 323:
		if (!fe_is_chanwindow(sess->server))
			EMIT_SIGNAL(XP_TE_SERVTEXT, serv->server_session, text, word[1], word[2], nullptr, 0);
		else
			fe_chan_list_end(sess->server);
		break;
	...
	case 341:						// INVITE ACK
		EMIT_SIGNAL(XP_TE_UINVITE, sess, word[4], word[5], serv->servername, nullptr, 0);
		break;
	...
	case 406:	// WHOWAS error
		EMIT_SIGNAL(XP_TE_SERVTEXT, whois_sess, text, word[1], word[2], nullptr, 0);
		serv->inside_whois = 0;
		break;
	...
	case 375:	// motd start
		if (!prefs.skipmotd || serv->motd_skipped)
			EMIT_SIGNAL(XP_TE_MOTD, serv->server_session, text, nullptr, nullptr,
							nullptr, 0);
		break;
	...
	case 471:
		EMIT_SIGNAL(XP_TE_USERLIMIT, sess, word[4], nullptr, nullptr, nullptr, 0);
		break;

	case 473:
		EMIT_SIGNAL(XP_TE_INVITE, sess, word[4], nullptr, nullptr, nullptr, 0);
		break;

	case 474:
		EMIT_SIGNAL(XP_TE_BANNED, sess, word[4], nullptr, nullptr, nullptr, 0);
		break;

	case 475:
		EMIT_SIGNAL(XP_TE_KEYWORD, sess, word[4], nullptr, nullptr, nullptr, 0);
		break;
	...
	def:
		if (is_channel(serv, word[4]))
		{
			...
			EMIT_SIGNAL(XP_TE_SERVTEXT, realsess, text, word[1], word[2], nullptr, 0);
		}
		else
		{
			EMIT_SIGNAL(XP_TE_SERVTEXT, serv->server_session, text, word[1], word[2], nullptr, 0);
...
static void process_named_msg(session *sess, char *type, char *word[], char *word_eol[])
{
	...
	if (len == 4)
	{
		...
		case WORDL('K','I','L','L'):
			EMIT_SIGNAL(XP_TE_KILL, sess, nick, word_eol[5], nullptr, nullptr, 0);
			return;
	...
	else if (len >= 5)
	{
		...
		case WORDL('I','N','V','I'):
			...
			if (word[4][0] == ':')
				EMIT_SIGNAL(XP_TE_INVITED, sess, word[4] + 1, nick, serv->servername, nullptr, 0);
			else
				EMIT_SIGNAL(XP_TE_INVITED, sess, word[4], nick, serv->servername, nullptr, 0);
		...
		case WORDL('W','A','L','L'):
			...
			if (*text == ':')
				text++;
			EMIT_SIGNAL(XP_TE_WALLOPS, sess, nick, text, nullptr, nullptr, 0);
			return;
		}
	}
...
static void process_named_servermsg(session *sess, char *buf, char *rawname, char *word_eol[])
{
	...
	{
		EMIT_SIGNAL(XP_TE_SERVERERROR, sess, buf + 7, nullptr, nullptr, nullptr, 0);
		return;
	}
	if (!strncmp(buf, "NOTICE ", 7))
	{
		...
		if (*buf == ':')
			buf++;
		EMIT_SIGNAL(XP_TE_SERVNOTICE, sess, buf, sess->server->servername, nullptr, nullptr, 0);
		return;
	}

	EMIT_SIGNAL(XP_TE_SERVTEXT, sess, buf, sess->server->servername, rawname, nullptr, 0);
}
...
void proto_fill_her_up(server *serv)
{
	...
	serv->p_raw = irc_raw;
	serv->p_cmp = rfc_casecmp;	// can be changed by 005 in modes.cpp
}
