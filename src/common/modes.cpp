					...
					int end, char sign, char mode, int modes_per_line)
{
	...
	while (wpos < end)
	{
		...
		tbuf[len] = 0;	/* nullptr terminate for the strcat() to work */
					...
					char *chan, char *arg, int quiet, int is_324)
{
	...
	case '+':
		...
		case 'k':
			...
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANSETKEY, sess, nick, arg, nullptr, nullptr, 0);
			return;
		case 'l':
			sess->limit = atoi (arg);
			...
			fe_update_mode_buttons (sess, mode, sign);
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANSETLIMIT, sess, nick, arg, nullptr, nullptr, 0);
			return;
		case 'o':
			if (!quiet)
		...
		case 'h':
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANHOP, sess, nick, arg, nullptr, nullptr, 0);
			return;
		case 'v':
			if (!quiet)
		...
		case 'b':
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANBAN, sess, nick, arg, nullptr, nullptr, 0);
			return;
		case 'e':
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANEXEMPT, sess, nick, arg, nullptr, nullptr, 0);
			return;
		case 'I':
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANINVITE, sess, nick, arg, nullptr, nullptr, 0);
			return;
		}
		break;
	case '-':
		...
		case 'k':
			...
			fe_update_mode_buttons (sess, mode, sign);
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANRMKEY, sess, nick, nullptr, nullptr, nullptr, 0);
			return;
		case 'l':
			sess->limit = 0;
			fe_update_channel_limit (sess);
			fe_update_mode_buttons (sess, mode, sign);
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANRMLIMIT, sess, nick, nullptr, nullptr, nullptr, 0);
			return;
		case 'o':
			if (!quiet)
		...
		case 'h':
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANDEHOP, sess, nick, arg, nullptr, nullptr, 0);
			return;
		case 'v':
			if (!quiet)
		...
		case 'b':
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANUNBAN, sess, nick, arg, nullptr, nullptr, 0);
			return;
		case 'e':
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANRMEXEMPT, sess, nick, arg, nullptr, nullptr, 0);
			return;
		case 'I':
			if (!quiet)
				EMIT_SIGNAL (XP_TE_CHANRMINVITE, sess, nick, arg, nullptr, nullptr, 0);
			return;
		}
	}
...
genmode:
	...
	if (!quiet)
	{
		if (*arg)
		{
			char *buf = (char*)malloc(strlen(chan) + strlen(arg) + 2);
...
mode_print_grouped (session *sess, char *nick, mode_run *mr)
{
	...
	if (mr->op)
	{
		EMIT_SIGNAL (XP_TE_CHANOP, sess, nick, mr->op, nullptr, nullptr, 0);
		free (mr->op);
		mr->op = nullptr;
	}
	...
	if (mr->deop)
	{
		EMIT_SIGNAL (XP_TE_CHANDEOP, sess, nick, mr->deop, nullptr, nullptr, 0);
		free (mr->deop);
		mr->deop = nullptr;
	}
	...
	if (mr->voice)
	{
		EMIT_SIGNAL (XP_TE_CHANVOICE, sess, nick, mr->voice, nullptr, nullptr, 0);
		free (mr->voice);
		mr->voice = nullptr;
	}
	...
	if (mr->devoice)
	{
		EMIT_SIGNAL (XP_TE_CHANDEVOICE, sess, nick, mr->devoice, nullptr, nullptr, 0);
		free (mr->devoice);
		mr->devoice = nullptr;
	}
}
			...
			char *nick, int numeric_324)
{
	...
	mr.serv = serv;
	mr.op = mr.deop = mr.voice = mr.devoice = nullptr;
