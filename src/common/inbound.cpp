...
static session* inbound_open_dialog(server *serv, char *from)
{
	...
	// for playing sounds
	EMIT_SIGNAL(XP_TE_OPENDIALOG, sess, nullptr, nullptr, nullptr, nullptr, 0);
...
void inbound_privmsg(server *serv, char *from, char *ip, char *text, int id)
{
	...
	if (sess || prefs.autodialog)
	{
		...
		inbound_chanmsg(serv, nullptr, nullptr, from, text, FALSE, id);
		return;
	}
	...
	if (!sess)
	{
		...
		EMIT_SIGNAL(XP_TE_PRIVMSG, sess, from, text, idtext, nullptr, 0);
		return;
	}

	if (sess->type == SESS_DIALOG)
		EMIT_SIGNAL(XP_TE_DPRIVMSG, sess, from, text, idtext, nullptr, 0);
	else
		EMIT_SIGNAL(XP_TE_PRIVMSG, sess, from, text, idtext, nullptr, 0);
}
...
bool alert_match_word(char *word, char *masks)
{
	char *p = masks;
	char endchar;
	int res;
...
bool alert_match_text(char *text, char *masks)
{
...
void inbound_action(session *sess, char *chan, char *from, char *ip, char *text, int fromme, int id)
{
	...
	if (fromme)
		...
		EMIT_SIGNAL(XP_TE_DPRIVACTION, sess, from, text, idtext, nullptr, 0);
	else
		EMIT_SIGNAL(XP_TE_PRIVACTION, sess, from, text, idtext, nullptr, 0);
}
...
void inbound_chanmsg(server *serv, session *sess, char *chan, char *from, char *text, char fromme, int id)
{
	...
	if (fromme)
	{
		...
		EMIT_SIGNAL(XP_TE_UCHANMSG, sess, from, text, nickchar, nullptr, 0);
		return;
	}
	...
	while (list)
	{
		...
		if (sess->server == serv)
		{
			if (userlist_change(sess, nick, newnick) || (me && sess->type == SESS_SERVER))
			{
				...
				{
					if (me)
						EMIT_SIGNAL(XP_TE_UCHANGENICK, sess, nick, newnick, nullptr, nullptr, 0);
					else
						EMIT_SIGNAL(XP_TE_CHANGENICK, sess, nick, newnick, nullptr, nullptr, 0);
				}
			}
...
static session* find_session_from_waitchannel(char *chan, server *serv)
{
	session *sess;
	GSList *list = sess_list;
	while (list)
...
void inbound_ujoin(server *serv, char *chan, char *nick, char *ip)
{
	...
	EMIT_SIGNAL(XP_TE_UJOIN, sess, nick, chan, ip, nullptr, 0);
...
void inbound_upart(server *serv, char *chan, char *ip, char *reason)
{
	...
	if (sess)
	{
		if (*reason)
			...
			EMIT_SIGNAL(XP_TE_UPART, sess, serv->nick, ip, chan, nullptr, 0);
		clear_channel(sess);
	}
}
...
void inbound_nameslist(server *serv, char *chan, char *names)
{
	...
	if (!sess)
	{
		EMIT_SIGNAL(XP_TE_USERSONCHAN, serv->server_session, chan, names, nullptr, nullptr, 0);
		return;
	}
	if (!sess->ignore_names)
		EMIT_SIGNAL(XP_TE_USERSONCHAN, sess, chan, names, nullptr, nullptr, 0);
...
void inbound_topic(server *serv, char *chan, char *topic_text)
{
	...
	EMIT_SIGNAL(XP_TE_TOPIC, sess, chan, topic_text, nullptr, nullptr, 0);
}
...
void inbound_topicnew(server *serv, char *nick, char *chan, char *topic)
{
	...
	if (sess)
	{
		...
		free(stripped_topic);
		EMIT_SIGNAL(XP_TE_NEWTOPIC, sess, nick, topic, chan, nullptr, 0);
	}
}
...
void inbound_join(server *serv, char *chan, char *user, char *ip)
{
	...
	if (sess)
	{
		EMIT_SIGNAL(XP_TE_JOIN, sess, user, chan, ip, nullptr, 0);
...
void inbound_part(server *serv, char *chan, char *user, char *ip, char *reason)
{
	...
	if (sess)
	{
		if (*reason)
			...
			EMIT_SIGNAL(XP_TE_PART, sess, user, ip, chan, nullptr, 0);
		userlist_remove(sess, user);
	}
}
...
void inbound_topictime(server *serv, char *chan, char *nick, time_t stamp)
{
	...
	EMIT_SIGNAL(XP_TE_TOPICDATE, sess, chan, nick, tim, nullptr, 0);
}
...
void inbound_quit(server *serv, char *nick, char *ip, char *reason)
{
	...
	while (list)
	{
		...
		if (sess->server == serv)
		{
			...
			{
				EMIT_SIGNAL(XP_TE_QUIT, sess, nick, reason, ip, nullptr, 0);
...
void inbound_ping_reply(session *sess, char *timestring, char *from)
{
	...
	if (atol(timestring) == 0)
	{
		if (sess->server->lag_sent)
			...
			EMIT_SIGNAL(XP_TE_PINGREP, sess, from, "?", nullptr, nullptr, 0);
	}
	else
	{
		snprintf(outbuf, sizeof(outbuf), "%ld.%ld%ld", dif / 1000000, (dif / 100000) % 10, dif % 10);
		EMIT_SIGNAL(XP_TE_PINGREP, sess, from, outbuf, nullptr, nullptr, 0);
	}
}
...
void inbound_notice(server *serv, char *to, char *nick, char *msg, char *ip, int id)
{
	...
	if (server_notice)
		EMIT_SIGNAL(XP_TE_SERVNOTICE, sess, msg, nick, nullptr, nullptr, 0);
	else if (ptr)
		EMIT_SIGNAL(XP_TE_CHANNOTICE, sess, nick, to, msg, nullptr, 0);
	else
		EMIT_SIGNAL(XP_TE_NOTICE, sess, nick, msg, nullptr, nullptr, 0);
}
...
void inbound_away(server *serv, char *nick, char *msg)
{
	...
	if (!serv->inside_whois || !serv->skip_next_whois)
		EMIT_SIGNAL(XP_TE_WHOIS5, sess, nick, msg, nullptr, nullptr, 0);
...
static bool check_autojoin_channels(server *serv)
{
	char *po;
	session *sess;
	GSList *list = sess_list;
...
void inbound_next_nick(session *sess, char *nick)
{
	...
	default:
		EMIT_SIGNAL(XP_TE_NICKFAIL, sess, nullptr, nullptr, nullptr, nullptr, 0);
		return;
	}

	net = (ircnet*)serv->network;
	// use network specific "XX choice"?
	if (net && !(net->flags & FLAG_USE_GLOBAL) && net->nick[serv->nickcount-1])
		newnick = net->nick[serv->nickcount-1];
	serv->p_change_nick(serv, newnick);
	EMIT_SIGNAL(XP_TE_NICKCLASH, sess, nick, newnick, nullptr, nullptr, 0);
}
...
void do_dns(session *sess, char *nick, char *host)
{
	...
	if (po)
		host = po + 1;
	EMIT_SIGNAL(XP_TE_RESOLVINGUSER, sess, nick, host, nullptr, nullptr, 0);
...
void inbound_foundip(session *sess, char *ip)
{
	...
	if (HostAddr)
	{
		prefs.dcc_ip = ((in_addr*)HostAddr->h_addr)->s_addr;
		EMIT_SIGNAL(XP_TE_FOUNDIP, sess, inet_ntoa(*((in_addr*)HostAddr->h_addr)), nullptr, nullptr, nullptr, 0);
	}
}
						...
						unsigned int away)
{
	...
	if (user && host)
	{
		uhost = (char*)malloc(strlen(user) + strlen(host) + 2);
...
{
	handle_command((session*)sess, (str[0] == '/') ? str + 1 : str, TRUE);
	return 1;
}
...
void inbound_login_end(session *sess, char *text)
{
	...
	if (prefs.skipmotd && !serv->motd_skipped)
	{
		...
		EMIT_SIGNAL(XP_TE_MOTDSKIP, serv->server_session, nullptr, nullptr, nullptr, nullptr, 0);
		return;
	}
	EMIT_SIGNAL(XP_TE_MOTD, serv->server_session, text, nullptr, nullptr, nullptr, 0);
}
