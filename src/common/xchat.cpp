...
void lag_check()
{
	...
	while (list)
	{
		...
		if (serv->connected && serv->end_of_motd)
		{
			...
			{
				sprintf(tbuf, "%d", lag);
				EMIT_SIGNAL(XP_TE_PINGTIMEOUT, serv->server_session, tbuf, nullptr, nullptr, nullptr, 0);
...
static int xchat_misc_checks()		// this gets called every 1/2 second
{
	static int count = 0;
#ifdef USE_MSPROXY
	static int count2 = 0;
...
static void irc_init(session *sess)
{
	...
	plugin_add(sess, nullptr, nullptr, (void*)timer_plugin_init, nullptr, nullptr, FALSE);
...
void session_free(session *killsess)
{
	...
	if (current_sess == killsess)
	{
		...
		if (sess_list)
			current_sess = (session*)sess_list->data;
	}
...
static int xchat_auto_connect(void* userdata)
{
...
void xchat_init()
{
	...
	{
		MessageBox(nullptr, "Cannot find winsock 2.2+", "Error", MB_OK);
		exit(0);
	}
#else
	...
	snprintf(buf, sizeof(buf),
		...
		_("_Open Dialog Window"), "PChat-dialog",
	...
	if (!arg_dont_autoconnect)
	{
		...
		{
			fe_idle_add((void*)xchat_auto_connect, nullptr);
		}
	}
	else
	{
...
static int child_handler(void* userdata)
{
	intptr_t pid = (intptr_t)userdata;
...
void xchat_exec(const char *cmd)
{
	...
	/* zombie avoiding system. Don't ask! it has to be like this to work
	   with zvt (which overrides the default handler)*/
		fe_timeout_add(1000, (void*)child_handler, (void*)pid);
#endif
}
