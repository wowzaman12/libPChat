...
static GSList *timer_list = nullptr;
...
static void timer_add(int ref, float timeout, int repeat, char *command)
{
	...
	tim->hook = xchat_hook_timer(ph, timeout * 1000.0, (callback)timeout_cb, tim);
	timer_list = g_slist_append(timer_list, tim);
}

static void timer_showlist()
{
	GSList *list;
	timer *tim;
