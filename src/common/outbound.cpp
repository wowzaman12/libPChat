...
static int cmd_back(session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (sess->server->is_away)
	{
		...
		if (prefs.show_away_message)
		{
			gone = time(nullptr) - sess->server->away_time;
	...
	if (sess->server->last_away_reason)
		free(sess->server->last_away_reason);
	sess->server->last_away_reason = nullptr;
...
static void exec_check_process(session *sess)
{
	...
	if (sess->running_exec == nullptr)
		return;
	val = waitpid(sess->running_exec->childpid, nullptr, WNOHANG);
	if (val == -1 || val > 0)
	{
...
static int cmd_newserver(session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	{
		new_ircwindow(nullptr, word[3], SESS_SERVER, 0);
		return TRUE;
	}

	sess = new_ircwindow(nullptr, nullptr, SESS_SERVER, 0);
...
static int cmd_notice(session *sess, char *tbuf, char *word[], char *word_eol[])
{
	if (*word[2] && *word_eol[3])
	{
		...
		EMIT_SIGNAL(XP_TE_NOTICESEND, sess, word[2], word_eol[3], nullptr, nullptr, 0);
		return TRUE;
	}
	return FALSE;
}
...
static int cmd_tray(session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (!word[3][0])
	{
		fe_tray_set_file(nullptr);	// default xchat icon
		return TRUE;
	}
	...
	fe_tray_set_flash(word[4], word[5][0] ? word[5] : nullptr, atoi(word[3]));
	return TRUE;
}
...
static int cmd_url(session *sess, char *tbuf, char *word[], char *word_eol[])
{
	if (word[2][0])
	{
		...
		if (parse_irc_url(url, &server_name, &port, &channel, &use_ssl))
		{
			...
			// check for "FreeNode"
			net = servlist_net_find(server_name, nullptr, strcasecmp);
...
{
	// the "-1" is to skip the nullptr terminator
...
static void handle_say(session *sess, char *text, int check_spch)
{
	...
	if (sess->server->connected)
	{
		...
		if (strlen(text) > max)
		{
			...
			max = i;
			t = text[max];
			text[max] = 0;			  // insert a nullptr terminator to shorten it
		}
