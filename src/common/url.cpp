...
void url_save(const char *fname, const char *mode, bool fullpath)
{
	FILE *fd;
	...
	tree_foreach((tree*)url_tree, (tree_traverse_func*)url_save_cb, fd);
	fclose(fd);
}
...
static void url_add(char *urltext, int len)
{
	...
	if (!url_tree)
		url_tree = tree_new((tree_cmp_func*)strcasecmp, nullptr);

	tree_insert((tree*)url_tree, data);
	fe_url_add(data);
}
