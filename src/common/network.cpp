...
netstore* net_store_new()
{
	...
	ns = (netstore*)malloc(sizeof(netstore));
...
{
	*sok_return = sok4;
	return connect(sok4, (sockaddr*)&ns->addr, sizeof(ns->addr));
}

void net_bind(netstore* tobindto, int sok4, int sok6)
{
	bind(sok4, (sockaddr*)&tobindto->addr, sizeof(tobindto->addr));
}
...
void net_store_fill_v4(netstore *ns, unsigned int addr, int port)
{
	ns->addr.sin_family = AF_INET;
...
unsigned int net_getsockaddr_v4(netstore *ns)
{
...
int net_getsockport(int sok4, int sok6)
{
	...
	if (getsockname(sok4, (sockaddr*)&addr, (socklen_t*)&len) == -1)
		return -1;
...
char* net_resolve(netstore* ns, char *hostname, int port, char **real_host)
{
	...
	if (port == 0)
		ret = getaddrinfo(hostname, nullptr, &hints, &ns->ip6_hostent);
	else
	...
	getnameinfo(ns->ip6_hostent->ai_addr, ns->ip6_hostent->ai_addrlen,
		ipstring, sizeof(ipstring), nullptr, 0, NI_NUMERICHOST);
