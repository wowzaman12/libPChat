...
#ifdef USE_DCC64
#ifdef _MSC_VER
#define BIG_STR_TO_INT(x) _strtoui64(x,nullptr,10)
#define stat _stat64
#else
...
static DCC *new_dcc();
static void dcc_close(DCC *dcc, int dccstat, int destroy);
static bool dcc_send_data(GIOChannel*, GIOCondition, DCC*);
static bool dcc_read(GIOChannel*, GIOCondition, DCC*);
static bool dcc_read_ack(GIOChannel *source, GIOCondition condition, DCC *dcc);
...
static void dcc_unthrottle(DCC *dcc)
{
	// don't unthrottle here, but delegate to funcs
	if (dcc->type == TYPE_RECV)
		dcc_read(nullptr, (GIOCondition)0, dcc);
	else
		dcc_send_data(nullptr, (GIOCondition)0, dcc);
}

static void dcc_calc_cps(DCC *dcc)
{
	GTimeVal now;
	int oldcps;
...
static void dcc_remove_from_sum(DCC *dcc)
{
		...
		case STAT_ACTIVE:
			...
			if (dcc->type == TYPE_SEND || dcc->type == TYPE_RECV)
			{
				if (prefs.dccstalltimeout > 0)
				{
					...
					{
						EMIT_SIGNAL(XP_TE_DCCSTALL, dcc->serv->front_session, dcctypes[dcc->type], file_part(dcc->file), dcc->nick, nullptr, 0);
		...
		case STAT_QUEUED:
			if (dcc->type == TYPE_SEND || dcc->type == TYPE_CHATSEND)
			{
				if (tim - dcc->offertime > prefs.dcctimeout)
				{
					...
					{
						EMIT_SIGNAL(XP_TE_DCCTOUT, dcc->serv->front_session, dcctypes[dcc->type], file_part(dcc->file), dcc->nick, nullptr, 0);
						dcc_close(dcc, STAT_ABORTED, FALSE);
					}
				}
			}
...
{
	struct hostent *h;
	static char *cache_host = nullptr;
...
static void
dcc_close (DCC *dcc, int dccstat, int destroy)
{
	if (dcc->wiotag)
	{
...
void
dcc_abort (session *sess, DCC *dcc)
{
	if (dcc)
	{
		switch (dcc->dccstat)
...
DCC* dcc_write_chat(char *nick, char *text)
{
	...
	{
		len = strlen (text);
		tcp_send_real (nullptr, dcc->sok, dcc->serv->encoding, dcc->serv->using_irc,
							text, len);
		send (dcc->sok, "\n", 1, 0);
		dcc->size += len;
		fe_dcc_update (dcc);
...
static int
dcc_chat_line (DCC *dcc, char *line)
{
	session *sess;
	char *word[PDIWORDS];
	char *po;
	...
	if (dcc->serv->using_irc) /* using "IRC" encoding (CP1252/UTF-8 hybrid) */
		utf = nullptr;
	else if (dcc->serv->encoding == nullptr)     /* system */
		utf = g_locale_to_utf8 (line, len, nullptr, &utf_len, nullptr);
	else
...
static bool dcc_read_chat(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	int i, len, dead;
	char portbuf[32];
	char lbuf[2050];
...
static bool dcc_read(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	char *old;
	char buf[4096];
	int n;
	...
	if (dcc->fp == -1)
	{
		...
		else
		{
			if (access(dcc->destfile_fs, F_OK) == 0)
			{
				...
				EMIT_SIGNAL(XP_TE_DCCRENAME, dcc->serv->front_session, old, dcc->destfile, nullptr, nullptr, 0);
				g_free(old);
			}
			dcc->fp = open(dcc->destfile_fs, OFLAGS | O_TRUNC | O_WRONLY | O_CREAT, prefs.dccpermissions);
		}
	}
	if (dcc->fp == -1)
	{
	...
	while (1)
	{
		...
		if (!dcc->iotag)
			dcc->iotag = fe_input_add(dcc->sok, FIA_READ|FIA_EX, (void*)dcc_read, dcc);
...
static bool dcc_did_connect(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	int er;
	struct sockaddr_in addr;
...
static bool dcc_connect_finished(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	char host[128];
	...
	case TYPE_RECV:
		dcc->iotag = fe_input_add(dcc->sok, FIA_READ|FIA_EX, (void*)dcc_read, dcc);
		EMIT_SIGNAL(XP_TE_DCCCONRECV, dcc->serv->front_session, dcc->nick, host, dcc->file, nullptr, 0);
		break;
	case TYPE_SEND:
		/* passive send */
		dcc->fastsend = prefs.fastdccsend;
		if (dcc->fastsend)
			dcc->wiotag = fe_input_add(dcc->sok, FIA_WRITE, (void*)dcc_send_data, dcc);
		dcc->iotag = fe_input_add(dcc->sok, FIA_READ|FIA_EX, (void*)dcc_read_ack, dcc);
		dcc_send_data(nullptr, (GIOCondition)0, dcc);
		EMIT_SIGNAL(XP_TE_DCCCONSEND, dcc->serv->front_session, dcc->nick, host, dcc->file, nullptr, 0);
		break;
	case TYPE_CHATSEND:	/* pchat */
		dcc_open_query(dcc->serv, dcc->nick);
	case TYPE_CHATRECV:	/* normal chat */
		dcc->iotag = fe_input_add(dcc->sok, FIA_READ|FIA_EX, (void*)dcc_read_chat, dcc);
		dcc->dccchat = (struct dcc_chat*)malloc(sizeof(struct dcc_chat));
		dcc->dccchat->pos = 0;
		EMIT_SIGNAL(XP_TE_DCCCONCHAT, dcc->serv->front_session, dcc->nick, host, nullptr, nullptr, 0);
		break;
	}
...
static bool read_proxy(DCC *dcc)
{
...
static bool write_proxy(DCC *dcc)
{
	proxy_state *proxy = dcc->proxy;
	while (proxy->bufferused < proxy->buffersize)
	{
...
static bool proxy_read_line(DCC *dcc)
{
	struct proxy_state *proxy = dcc->proxy;
	while (1)
	{
...
static bool dcc_wingate_proxy_traverse(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	struct proxy_state *proxy = dcc->proxy;
	if (proxy->phase == 0)
	{
		...
		dcc->wiotag = fe_input_add(dcc->sok, FIA_WRITE|FIA_EX,
									(void*)dcc_wingate_proxy_traverse, dcc);
		++proxy->phase;
	}
	if (proxy->phase == 1)
	{
		...
		fe_input_remove(dcc->wiotag);
		dcc->wiotag = 0;
		dcc_connect_finished(source, (GIOCondition)0, dcc);
	}
	return TRUE;
}
...
static bool dcc_socks_proxy_traverse(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	proxy_state *proxy = dcc->proxy;
	...
	if (proxy->phase == 0)
	{
		...
		proxy->bufferused = 0;
		dcc->wiotag = fe_input_add(dcc->sok, FIA_WRITE|FIA_EX,
									(void*)dcc_socks_proxy_traverse, dcc);
		++proxy->phase;
	}
...
static bool dcc_socks5_proxy_traverse(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	proxy_state *proxy = dcc->proxy;
	...
	if (proxy->phase == 0)
	{
		...
		proxy->bufferused = 0;
		dcc->wiotag = fe_input_add(dcc->sok, FIA_WRITE|FIA_EX,
									(void*)dcc_socks5_proxy_traverse, dcc);
		++proxy->phase;
	}
...
static bool dcc_http_proxy_traverse(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	proxy_state *proxy = dcc->proxy;
	...
	if (proxy->phase == 0)
	{
		...
		dcc->wiotag = fe_input_add(dcc->sok, FIA_WRITE|FIA_EX,
									(void*)dcc_http_proxy_traverse, dcc);
		++proxy->phase;
	}
...
static bool dcc_proxy_connect(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	fe_input_remove(dcc->iotag);
	dcc->iotag = 0;
	...
	dcc->proxy = (proxy_state*)malloc(sizeof(proxy_state));
	if (!dcc->proxy)
	{
		dcc->dccstat = STAT_FAILED;
		fe_dcc_update(dcc);
...
static int dcc_listen_init(DCC*, session*);
...
dcc_connect (DCC *dcc)
{
	...
	else
	{
		...
		if (DCC_USE_PROXY ())
			dcc->iotag = fe_input_add (dcc->sok, FIA_WRITE|FIA_EX, (void*)dcc_proxy_connect, dcc);
		else
			dcc->iotag = fe_input_add (dcc->sok, FIA_WRITE|FIA_EX, (void*)dcc_connect_finished, dcc);
	}

	fe_dcc_update (dcc);
}

static bool dcc_send_data(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	char *buf;
	...
	buf = (char*)malloc(prefs.dcc_blocksize);
	if (!buf)
		return TRUE;
...
static bool dcc_handle_new_ack(DCC *dcc)
{
	unsigned int ack;
	char buf[16];
	bool done = false;
	...
	if (dcc->pos >= dcc->size && dcc->ack >= (dcc->size & 0xffffffff))
	{
		...
		dcc_calc_average_cps(dcc);	/* this must be done _after_ dcc_close, or dcc_remove_from_sum will see the wrong value in dcc->cps */
		sprintf(buf, "%d", dcc->cps);
		EMIT_SIGNAL(XP_TE_DCCSENDCOMP, dcc->serv->front_session, file_part(dcc->file), dcc->nick, buf, nullptr, 0);
		done = TRUE;
	}
...
static bool dcc_accept(GIOChannel *source, GIOCondition condition, DCC *dcc)
{
	char host[128];
	sockaddr_in CAddr;
	int sok;
	...
	len = sizeof(CAddr);
	sok = accept(dcc->sok, (sockaddr*)&CAddr, &len);
	fe_input_remove(dcc->iotag);
	dcc->iotag = 0;
	closesocket(dcc->sok);
	if (sok < 0)
	...
	if (dcc->pasvid)
		return dcc_connect_finished(nullptr, (GIOCondition)0, dcc);
...
unsigned int dcc_get_my_address()	// the address we'll tell the other person
{
...
static int dcc_listen_init(DCC *dcc, session *sess)
{
	...
	len = sizeof(SAddr);
	getsockname(dcc->serv->sok, (sockaddr*)&SAddr, &len);
	...
	if (prefs.first_dcc_send_port > 0)
	{
			...
			(bindretval == -1))
		{
			...
			bindretval = bind(dcc->sok, (sockaddr*)&SAddr, sizeof(SAddr));
		}
	...
	len = sizeof(SAddr);
	getsockname(dcc->sok, (sockaddr*)&SAddr, &len);
...
void
dcc_send (session *sess, char *to, char *file, int maxcps, int passive)
{
	char outbuf[512];
	struct stat st;
	DCC *dcc;
...
static DCC* find_dcc_from_id(int id, int type)
{
	DCC *dcc;
	GSList *list = dcc_list;
	while (list)
...
static DCC* find_dcc_from_port(int port, int type)
{
	DCC *dcc;
	GSList *list = dcc_list;
	while (list)
...
void
dcc_change_nick (server *serv, char *oldnick, char *newnick)
{
	DCC *dcc;
	GSList *list = dcc_list;
...
static int
is_same_file (DCC *dcc, DCC *new_dcc)
{
...
static int is_resumable(DCC *dcc)
{
	dcc->resumable = 0;
...
void
dcc_get_with_destfile (DCC *dcc, char *file)
{
	g_free (dcc->destfile);
...
void
dcc_get_nick (session *sess, char *nick)
{
	DCC *dcc;
	GSList *list = dcc_list;
	while (list)
...
void
dcc_chat (session *sess, char *nick, int passive)
{
	char outbuf[512];
	DCC *dcc;
...
static void
dcc_malformed (session *sess, char *nick, char *data)
{
...
static DCC* dcc_add_chat(session *sess, char *nick, int port, unsigned int addr, int pasvid)
{
	DCC *dcc;
...
static DCC* dcc_add_file(session *sess, char *file, DCC_SIZE size, int port, char *nick, unsigned int addr, int pasvid)
{
	DCC *dcc;
	char tbuf[512];
	...
	if (dcc)
	{
		...
		dcc->destfile = (char*)g_malloc(strlen(prefs.dccdir) + strlen(nick) +
										strlen(file) + 4);
...
void handle_dcc(session *sess, char *nick, char *word[], char *word_eol[])
{
	...
	if (!strcasecmp(type, "Resume"))
	{
		...
		if (dcc)
		{
			...
			EMIT_SIGNAL(XP_TE_DCCRESUMEREQUEST, sess, nick, file_part (dcc->file), tbuf, nullptr, 0);
		}
		return;
	}
	...
	if (!strcasecmp(type, "SEND"))
	{
		...
		else if (word[10][0] != 0)
		{
			...
			* because this field is always nullptr (no pasvid)
			* in normal dcc sends.
			*/
			pasvid = atoi(word[10]);
			psend = 1;
	...
	{
		EMIT_SIGNAL(XP_TE_DCCGENERICOFFER, sess->server->front_session, word_eol[4] + 2, nick, nullptr, nullptr, 0);
	}
}

void
dcc_show_list (session *sess)
{
	int i = 0;
	DCC *dcc;
	GSList *list = dcc_list;
