...
void list_addentry(GSList** list, char *cmd, char *name)
{
	...
	pop = (struct popup*)malloc(sizeof(struct popup) + cmd_len + name_len);
...
char *xdir_fs = nullptr;	// file system encoding
char *xdir_utf = nullptr;	// utf-8 encoding

#ifdef WIN32

#include <windows.h>

static bool get_reg_str(const char *sub, const char *name, char *out, DWORD len)
{
	HKEY hKey;
	DWORD t;

	if (RegOpenKeyExA(HKEY_CURRENT_USER, sub, 0, KEY_READ, &hKey) == ERROR_SUCCESS)
	{
		if (RegQueryValueExA(hKey, name, nullptr, &t, (BYTE*)out, &len) != ERROR_SUCCESS || t != REG_SZ)
		{
			RegCloseKey(hKey);
			return false;
		}
...
#endif
			fe_message(_("Cannot create ~/.pchat"), FE_MSG_ERROR);
	}
}
...
static char* default_file()
{
	...
	if (!dfile)
	{
		dfile = (char*)malloc(strlen(get_xdir_fs()) + 12);
...
const struct prefs vars[] =
{
	...
	{"pchat_reconnect_join_chan", P_OFFINT(rejoinchans), TYPE_BOOL},
...
void load_config()
{
	...
	if (fh != -1)
	{
		...
		if (i >= 0)
			cfg[i] = '\0';					// make sure cfg is nullptr terminated
		close(fh);
		i = 0;
		do
		{
...
int save_config()
{
	...
	config = default_file();
	new_config = (char*)malloc(strlen(config) + 5);
