...
/* globals */
static chiper_info chp_info;		/* static buffer for _SSL_get_cipher_info() */
...
SSL_CTX* _SSL_context_init(void (*info_cb_func), int server)
{
	...
	SSL_CTX_set_info_callback(ctx, (info_cb)info_cb_func);
...
chiper_info* _SSL_get_cipher_info(SSL* ssl)
{
	SSL_CIPHER *c;


	c = (SSL_CIPHER*)SSL_get_current_cipher (ssl);
	strncpy (chp_info.version, SSL_CIPHER_get_version (c),
				sizeof (chp_info.version));
	strncpy (chp_info.chiper, SSL_CIPHER_get_name (c),
				sizeof (chp_info.chiper));
	SSL_CIPHER_get_bits (c, &chp_info.chiper_bits);
...
typedef int (*cb)(int, X509_STORE_CTX*);
...
*/
	SSL_CTX_set_verify(ctx, SSL_VERIFY_PEER, (cb)verify_callback);
