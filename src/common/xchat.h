...
// force a 32bit CMP.L
#define CMPL(a, c0, c1, c2, c3) (a == (c0 | (c1 << 8) | (c2 << 16) | (c3 << 24)))
#define WORDL(c0, c1, c2, c3) (c0 | (c1 << 8) | (c2 << 16) | (c3 << 24))
#define WORDW(c0, c1) (c0 | (c1 << 8))
...
#define NICKLEN		64				// including the nullptr, so 63 really
#define CHANLEN		300
#define PDIWORDS	32
...
{
	// server control operations (in server*.cpp)
	...
	// irc protocol functions (in proto*.cpp)
	...
	void *network;						// points to entry in servlist.c or nullptr!
	...
	session *front_session;		// front-most window/tab
	session *server_session;	// server window/tab
	...
	char *encoding;					// nullptr for system
...
typedef int (*cmd_callback)(session* sess, char *tbuf, char *word[], char *word_eol[]);
